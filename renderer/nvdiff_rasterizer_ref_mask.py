...
@threestudio.register("dreamcraft3d-mask-nvdiff-rasterizer")
        ...
        **kwargs
        ...
        if render_mask:
            ...
            with torch.no_grad():
                ...
                rast_front, _ = self.ctx.rasterize(
                    v_pos_clip_front, mesh.t_pos_idx, (height, width)
                )
                mask_front = rast_front[..., 3:]
                mask_front = mask_front[mask_front > 0] - 1.0
                faces_vis = mesh.t_pos_idx[mask_front.long()]

                mesh._v_rgb = torch.zeros(mesh.v_pos.shape[0], 1).to(mesh.v_pos)
                mesh._v_rgb[faces_vis[:, 0]] = 1.0
                mesh._v_rgb[faces_vis[:, 1]] = 1.0
                mesh._v_rgb[faces_vis[:, 2]] = 1.0
                mask_vis, _ = self.ctx.interpolate_one(
                    mesh._v_rgb, rast, mesh.t_pos_idx
                )
                mask_vis = mask_vis > 0.0
