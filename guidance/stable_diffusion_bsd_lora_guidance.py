...
@threestudio.register("dreamcraft3d-stable-diffusion-bsd-lora-guidance")
class StableDiffusionBSDGuidance(BaseModule):
    ...
    def configure(self) -> None:
        ...
        self.unet_lora.class_embedding = self.camera_embedding_lora
        # set up LoRA layers
        lora_attn_procs = {}
    ...
    def _sample(
        ...
        for i, t in enumerate(timesteps[t_start:]):
            ...
            latents = sample_scheduler.step(
                noise_pred.to(latents), t, latents
            ).prev_sample
    ...
    def compute_grad_vsd(
        ...
        if self.scheduler_lora.config.prediction_type == "v_prediction":
            ...
            # todo
            # noise_pred_pretrain = latent_model_input * torch.cat([sigma_t] * 2, dim=0).view(
            #     -1, 1, 1, 1
            # ) + noise_pred_pretrain * torch.cat([alpha_t] * 2, dim=0).view(-1, 1, 1, 1)
        ...
        # sds_grad = w * (noise_pred_pretrain - noise)

        # low_sds_grad = F.interpolate(F.interpolate(sds_grad, (H // 4, W // 4), mode='bilinear'), (H, W), mode='bilinear')
        # low_grad = F.interpolate(F.interpolate(grad, (H // 4, W // 4), mode='bilinear'), (H, W), mode='bilinear')
        # high_grad = grad - low_grad
        return grad
    ...
    def train_pretrain(
        ...
        if sample_new_img or len(self.cache_frames) == 0:
            with torch.no_grad():
                ...
                save_image(
                    pretrain_images_sample, f".threestudio_cache/test_pretrain.jpg"
                )
        if len(self.cache_frames) > 10:
            self.cache_frames.pop(0)
        random_idx = torch.randint(0, len(self.cache_frames), [1]).item()
        images_sample = self.cache_frames[random_idx].detach().float()
